...
Apparently, when you _don't_ use App Router (recommended), the pages directory _is available_ and on the contrary, when using the App Router, the pages directory _isn't available_.

The App Router is _new_ and I'll have to explore using the App Router in a future webLog without the pages directory, out the box, and see how things work alternatively.
...
```js:/pages/index.js showLineNumbers
...
16. Then, replaced **Inter** with **Raleway** in the import statement:
```diff:/pages/index.js
...
21. Next, I went back to the Tailwind CSS framework guide, copied the next command, and created a Tailwind CSS configuration file and Post CSS configuration file by pasting, and executing the command:
```bash:terminal
npx tailwindcss init -p
```

22. At this point, the following Tailwind CSS configuration file and Post CSS configuration file were now available in the root of the project folder:
```plaintext
nextjs-tailwindcss/tailwind.config.js
nextjs-tailwindcss/postcss.config.js
```
...
```js:/nextjs-tailwindcss/tailwind.config.js {4-9}
...
```css:/nextjs-tailwindcss/styles/globals.css
@tailwind base;
@tailwind components;
@tailwind utilities;
```
